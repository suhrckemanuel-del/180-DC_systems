/* Headless structural verification for DELFT — "Ochtendlicht".
...
/* ---- W2: NaN / range sweep -------------------------------------- */
{
  ...
  const inst = G.instBuf();
  for (let i = 0; i < inst.length; i++){ if (!Number.isFinite(inst[i])) { bad++; } }
  const wv = G.waterV();
...
/* ---- W5: determinism ---------------------------------------------- */
{
  ...
  G.buildWorld(); G.packWorld(); G.packInstances();
...
/* ---- C1: collision fuzz ------------------------------------------- */
{
  ...
  const spots = [];
  for (const r of G.RESPAWNS){ spots.push(r); }
  for (const L of G.LM){
    for (let a = 0; a < 8; a++){
...
{
  const S = G.GS, K = G.KEY;
  const walk = (steps) => { K.w = true; for (let i = 0; i < steps; i++) { G.step(T.STEP); } K.w = false; };
  const notes = [];

  /* across a bridge over the Oude Delft: start west, walk east.
     The deck patch must win over the canal bed the whole way. */
  /* start at the west end of the deck (bridge at x=-150, half-span 15) */
  G.spawn(-164, 2, Math.PI * 0.5);
  let wet = 0, overWater = 0;
  K.w = true;
  for (let i = 0; i < 1400; i++){
    G.step(T.STEP);
    if (S.py < T.WATER_Y + 0.2) { wet++; }
    if (S.px > -156 && S.px < -144) { overWater++; }
  }
  K.w = false;
  if (!(S.px > -138)) { notes.push(`bridge crossing stalled at x=${S.px.toFixed(1)}`); }
  if (overWater === 0) { notes.push('never passed over the canal span'); }
  if (wet > 0) { notes.push(`fell in while crossing the bridge (${wet} steps below water)`); }

  /* walking off a quay must put the player back on solid ground */
  G.spawn(-160, -95, Math.PI * 0.5);
  K.w = true;
  for (let i = 0; i < 900; i++){ G.step(T.STEP); }
  K.w = false;
  ...
  check('C2', notes.length === 0,
    `scripted traversals: bridge crossing and quay edge behave${notes.length ? ' — ' + notes.join('; ') : ''}`);
}
...
/* ---- S1: shader lint ---------------------------------------------- */
{
  const sh = G.shaders();
  const problems = [];
  const pairs = [['vsSolid','fsSolid'], ['vsInst','fsSolid'], ['vsWater','fsWater'], ['vsSky','fsSky']];
  for (const [vn, fn] of pairs){
    ...
    for (const src of [vs, fs]){
      const decl = new Set([...src.matchAll(/uniform\s+\w+\s+(\w+)\s*;/g)].map(x => x[1]));
...
/* ---- GL call sanity ------------------------------------------------ */
{
  check('P2', GLCALLS.bufferData <= 6 && GLCALLS.bufferData >= 2,
