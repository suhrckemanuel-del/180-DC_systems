...
No one-fix loops triggered — all three cleared ≥4 on every dimension on first pass. One a11y nit caught in review and fixed immediately: V3 and V7 both had `aria-current="page"` on the `#mission` nav link, which is meant for distinguishing separate pages, not a same-page anchor — removed from both. One pre-existing, out-of-scope item left untouched: V7's guide.html still references a `.stratum--sky2` CSS class dropped in an earlier (pre-rebrand) round — cosmetic only, confined to the guide, not part of this pass.
